...
class _EmailPageState extends BaseState<EmailPage> {
  final List<pulse.Email> _data = [];
  String _search = '*';
  ...
  Future _load() async {
    ...
    if (null != widget.to ||
      ...
      await execute(() async {
        var res = await TwinnedSession.instance.pulseAdmin.queryEmail(
      ...
      await execute(() async {
        var res = await TwinnedSession.instance.pulseAdmin.searchEmail(
            apikey: TwinnedSession.instance.authToken,
  ...
  void showEmailLogs(BuildContext context, pulse.Email emailData) {
    String prettyJson = const JsonEncoder.withIndent('  ').convert(emailData);
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          title: const Text('Email Logs Data'),
