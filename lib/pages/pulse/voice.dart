...
class _VoicePageState extends BaseState<VoicePage> {
  final List<pulse.Voice> _data = [];
  String _search = '*';
        ...
        cells: [
          DataCell(Row(
            children: [
              ...
              InkWell(
                onTap: () {
                  showVoiceLogs(context, e);
                },
                child: const Tooltip(
                    message: 'View Voice Logs Data',
  ...
  Future _load() async {
    ...
    if (null != widget.to ||
      ...
      await execute(() async {
        var res = await TwinnedSession.instance.pulseAdmin.queryVoice(
      ...
      await execute(() async {
        var res = await TwinnedSession.instance.pulseAdmin.searchVoice(
            apikey: TwinnedSession.instance.authToken,
  ...
  void showVoiceLogs(BuildContext context, pulse.Voice voiceData) {
    String prettyJson = const JsonEncoder.withIndent('  ').convert(voiceData);
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          title: const Text('Voice Logs Data'),
